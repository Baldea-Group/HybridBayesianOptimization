...
\begin{abstract}
We consider grey-box problems where the decision variables naturally partition into black-box variables $x^{\BB}$ (entering an expensive function) and white-box variables $x^{\WB}$ (governed by known equations), with the black-box depending only on $x^{\BB}$. We exploit this separability through a bilevel reformulation: the outer loop uses Bayesian optimization (BO) to optimize the scalar objective as a function of $x^{\BB}$ alone, while the inner loop implicitly solves the white-box subproblem via nonlinear programming (NLP). The Gaussian process surrogate therefore operates over $\mathbb{R}^{n_{\BB}}$ rather than $\mathbb{R}^{n_{\WB}+n_{\BB}}$, and white-box constraints are satisfied exactly whenever the inner NLP converges to a feasible point---without penalty functions, chance constraints, or moment approximations. On a suite of 13 benchmark problems bilevel BO achieves 14$\times$--277{,}000$\times$ lower regret than full-space BO across 7{,}410 independent optimization runs, with comparable wall time for 12 of 13 problems. The advantage grows with dimension---up to 277{,}000$\times$ on 5D problems---and is robust to initialization size and exploration parameters. When grey-box problems exhibit variable separability, solving the known subproblem exactly rather than surrogating it should be the default approach.
\end{abstract}
...
Surrogate-based optimization is the standard approach for expensive black-box functions~\cite{frazier2018tutorial}, yet many problems contain known, differentiable substructure that monolithic surrogates waste samples learning. When an optimized system decomposes into an expensive black-box component and a cheap white-box component, building a single surrogate over all variables forces the Gaussian process (GP) to learn both the unknown and the known simultaneously. The cost of this redundancy grows with dimension: as the combined variable space $[x^{\WB}, x^{\BB}]$ expands, the curse of dimensionality penalizes the surrogate for structure it should never have had to discover.

This situation arises naturally in multi-scale engineering problems where design and operation are jointly optimized. For example, a catalyst designer uses density functional theory (DFT) simulations to predict kinetic parameters, which are then used in well-understood reactor mass and energy balances to evaluate reaction yield. Similarly, a polymer engineer uses molecular simulations to estimate membrane transport properties, then solves the solution-diffusion equations to size a separation unit. In each case, the overall objective depends on two groups of decision variables. \emph{Black-box variables} $x^{\BB}$ are inputs to expensive simulations or experiments such as molecular descriptors, catalyst properties, or experimental parameters. Conversely, \emph{white-box variables} $x^{\WB}$ aer decision variables that interact with well-understood equations. The black-box and white-box components are \emph{separable}: the expensive computation depends only on $x^{\BB}$, producing outputs $y = f^{\BB}(x^{\BB})$ that parameterize the white-box model. The key structural property is that, for any fixed $x^{\BB}$ and $y$, the remaining optimization over $x^{\WB}$ is a standard NLP solvable by conventional methods.

Several lines of work exploit known structure in expensive optimization, ranging from deterministic grey-box solvers~\cite{boukouvala2017greybox_jgo,boukouvala2017argonaut,eason2016trf} to composite and structured Bayesian optimization~\cite{astudillo2019bocf,paulson2022cobalt,kudva2026bonsai} to data-driven bilevel methods~\cite{beykal2020domino,kieffer2017bilevel}. We review these in Section~\ref{sec:related_work}. The common gap is that no prior method combines a GP-based global search over only the black-box variables with exact NLP solution of the white-box subproblem---the configuration that fully exploits variable separability for both dimensionality reduction and exact constraint satisfaction.

When the white-box optimization is solvable exactly, the surrogate should operate over $\mathbb{R}^{n_{\BB}}$ rather than $\mathbb{R}^{n_{\WB}+n_{\BB}}$. We exploit this through a bilevel reformulation: the outer loop uses BO to search over $x^{\BB}$; for each candidate, we evaluate the black box to obtain intermediate outputs $y$, then solve the white-box optimization exactly via nonlinear programming (NLP) to get $x^{\WB\star}$ and $J(x^{\WB\star})$. The GP models only the scalar mapping $x^{\BB} \mapsto J(x^{\WB\star}, y, x^{\BB})$, reducing the surrogate dimension from $n_{\WB}+n_{\BB}$ to $n_{\BB}$ and satisfying white-box constraints exactly whenever the inner NLP converges to a feasible point.

We make three contributions:
\begin{enumerate}
    \item A bilevel reformulation that reduces surrogate dimensionality from $n_{\WB}+n_{\BB}$ to $n_{\BB}$ by solving the white-box subproblem exactly via NLP, with white-box constraints satisfied exactly whenever the inner NLP converges to a feasible point---without penalty functions, chance constraints, or moment approximations.
    \item A benchmark suite of 13 separable grey-box problems (2--5 total variables, 0--3 constraints, both min and max) spanning synthetic test functions and engineering domains---the largest such suite for this problem class.
    \item Comprehensive empirical evidence from 7{,}410 independent optimization runs (7{,}280 BO + 130 NLP): bilevel BO achieves 14$\times$--277{,}000$\times$ lower regret than black-box BO across all 13 problems, with comparable wall time and robustness to hyperparameters ($n_\text{init}$, $\xi$).
\end{enumerate}
...
Our bilevel framework sits at the intersection of surrogate-based global optimization, grey-box Bayesian optimization, bilevel programming, and constrained expensive optimization. We review each thread, focusing on the structural assumptions each approach makes and where those assumptions limit applicability to separable grey-box problems.

\subsection{Surrogate-Based Global Optimization}

The use of surrogate models to guide expensive optimization traces to the Efficient Global Optimization (EGO) algorithm of Jones et al.~\cite{jones1998efficient}, which introduced Expected Improvement (EI) as an acquisition function for Gaussian process surrogates. EGO treats the objective as a monolithic black box and builds a single GP over the full decision space---an approach that remains the default in most BO implementations~\cite{shahriari2016bo_survey,frazier2018tutorial}.

Within the deterministic surrogate tradition, Boukouvala, Hasan, and Floudas~\cite{boukouvala2017greybox_jgo} developed a methodology for constrained grey-box problems that selects surrogates from a diverse set of functional forms and globally optimizes the resulting NLP. The companion ARGONAUT framework~\cite{boukouvala2017argonaut} formalized this into an iterative algorithm with variable selection, bounds tightening, and constrained sampling for problems with up to 100 variables; Kieslich et al.~\cite{kieslich2018smolyak} extended the approach using Smolyak grids and polynomial surrogates. These methods share our philosophy of exploiting known equations, but use deterministic surrogates that lack the principled exploration--exploitation trade-off of probabilistic models and tend to overexploit early samples under tight evaluation budgets.

The trust-region filter algorithms of Eason and Biegler~\cite{eason2016trf,eason2018trf} offer a complementary local approach: they embed surrogates for black-box components within a trust region and solve the glass-box NLP at each iteration. This is architecturally close to our inner-loop NLP solve, but the trust-region framework provides only local convergence guarantees---no global exploration mechanism steers the search away from poor basins.

\subsection{Grey-Box Bayesian Optimization}

Grey-box BO methods incorporate known structure into the surrogate to reduce its modeling burden. Astudillo and Frazier~\cite{astudillo2019bocf} modeled composite functions $g(h(x))$ by placing a multi-output GP on the expensive inner function $h$ and propagating uncertainty through the known outer function $g$ via sampling; later extensions handle function networks~\cite{astudillo2021bofn} and partial evaluations~\cite{buathong2024partial}. COBALT~\cite{paulson2022cobalt} extends this to constrained grey-box problems by combining multivariate GP models with a constrained expected utility function, using sample average approximation and chance constraints to propagate GP uncertainty through known equations. Gonz\'{a}lez-Garay and del Rio-Chanona~\cite{gonzalez2025bois} propose an interconnected-systems BO framework with adaptive linearization, Winz et al.~\cite{winz2025greybox} modify the upper confidence bound to exploit grey-box derivative information, and BONSAI~\cite{kudva2026bonsai} exploits function-network topology for robust optimization.

All of these methods surrogate the \emph{intermediate} black-box outputs $y$ and propagate uncertainty through the white-box equations, requiring multi-output GPs and moment approximations or sampling. When the white-box subproblem is solvable exactly via NLP---as when $\dim(x^{\WB})$ is moderate---surrogating $y$ adds modeling complexity without clear benefit. We instead surrogate the \emph{optimal value} $J(x^{\WB\star}, y, x^{\BB})$ directly, which yields a scalar GP in $\mathbb{R}^{n_{\BB}}$ and avoids both multi-output surrogates and uncertainty propagation (Table~\ref{tab:method_comparison}).

\subsection{Bilevel Optimization}

Bilevel programming has a rich history in the global optimization literature. G\"{u}m\"{u}\c{s} and Floudas~\cite{gumus2001bilevel} developed the first rigorous global optimization approach for nonlinear bilevel problems using the $\alpha$BB framework. Mitsos, Chachuat, and Barton~\cite{mitsos2009bilevel_dynamic} extended this to bilevel dynamic optimization, Kleniati and Adjiman~\cite{kleniati2014bilevel} addressed the mixed-integer case, and Fai\c{s}ca et al.~\cite{faisca2007bilevel} proposed parametric global optimization for bilevel programs via multi-parametric programming. These exact methods provide optimality guarantees but require analytical access to both levels---an assumption that breaks when the upper level involves an expensive black-box simulation.
...
Within the BO literature, Kieffer et al.~\cite{kieffer2017bilevel} first applied BO to bilevel problems, using EI at the outer level and SLSQP at the inner level. This nested architecture is structurally identical to ours, but Kieffer et al.\ did not exploit grey-box structure or dimensionality reduction. Recent work addresses the harder setting where \emph{both} levels are black boxes: Ekmekcioglu et al.~\cite{ekmekcioglu2024bilevel} model both levels as GPs over the joint space; Chew et al.~\cite{chew2025bilbo} construct confidence-bound trusted sets with regret guarantees; Fu et al.~\cite{fu2024convergence} provide convergence analysis for BO bilevel problems where the inner loop is unconstrained SGD. When the inner problem is a known white-box NLP, solving it exactly is both cheaper and more accurate than learning a second surrogate.

Baldea~\cite{baldea2025multiscale} introduced the multi-scale bilevel BO framework we extend here, demonstrating it on a single CSTR case study. We advance that work with a 13-problem benchmark suite, with systematic hyperparameter sweeps, and detailed comparisons quantifying when and why bilevel decomposition helps.

\subsection{Constraint Handling in Expensive Optimization}

Standard constrained BO methods model constraints as expensive black-box functions, each requiring a separate GP. Gardner et al.~\cite{gardner2014} introduced weighted expected improvement for unknown constraints; Gelbart et al.~\cite{gelbart2014unknown} extended this with probability-of-feasibility weighting; Picheny et al.~\cite{picheny2016albo} proposed a slack-variable augmented Lagrangian approach. These methods are appropriate when constraint functions are truly unknown, but when constraints arise from known white-box equations---mass balances, safety envelopes, thermodynamic limits---surrogating them wastes evaluations and introduces approximation error.

Our inner NLP satisfies white-box constraints exactly, avoiding penalty functions, chance constraints, and moment approximations. The advantage is largest when feasible regions are tight: on Distillation and Heat-Exchanger, bilevel BO achieves $>$100{,}000$\times$ lower regret than penalty-based black-box BO (Section~\ref{sec:results}).
...
\begin{itemize}
    \item $x^{\WB} \in \mathbb{R}^{n_{\WB}}$: \emph{white-box variables} governed by known, differentiable equations (e.g., temperatures, pressures, flow rates in engineering; policy parameters in control; design dimensions in structural optimization)
    \item $x^{\BB} \in \mathbb{R}^{n_{\BB}}$: \emph{black-box variables} that parameterize an expensive, non-differentiable function (e.g., molecular descriptors requiring DFT, material properties from simulations, hyperparameters of a costly simulator)
\end{itemize}

Parameters $y$ are computed by a black-box model
\begin{equation}\label{eq:molecular_model}
    y = f^{\BB}(x^{\BB}).
\end{equation}
This function $f^{\BB}$ is expensive to evaluate; we lack closed-form expressions or derivative information for it.

The white-box model generally takes the implicit form
\begin{equation}\label{eq:process_model}
    f^{\WB}(x^{\WB}, y) = 0,
\end{equation}
where $y \in \mathbb{R}^{n_y}$ is a vector of parameters that enter the white-box equations. We assume $f^{\WB}$ is sufficiently smooth and that the Jacobian $\partial f^{\WB}/\partial x^{\WB}$ is available analytically.

The integrated optimization problem over both variable groups is
...
\begin{enumerate}
    ...
    \item[(ii)] For any fixed $x^{\BB}$ and $y = f^{\BB}(x^{\BB})$, the white-box subproblem $\min_{x^{\WB}} J(x^{\WB}, y, x^{\BB})$ subject to $f^{\WB}(x^{\WB}, y) = 0$, $g(x^{\WB}, y, x^{\BB}) \leq 0$, $x^{\WB} \in \mathcal{X}^{\WB}$ is solvable to global optimality (of the subproblem) by a deterministic NLP solver.
\end{enumerate}
\end{assumption}

Under Assumption~\ref{ass:separability}, the optimization decomposes: for any fixed $x^{\BB}$, the black-box evaluation $y = f^{\BB}(x^{\BB})$ and the inner NLP over $x^{\WB}$ are self-contained. This decoupling is the structure we exploit: rather than optimizing all variables jointly, we can search over the low-dimensional black-box space while solving the white-box subproblem exactly at each step. Note that $f^{\BB}$ can always be augmented with identity mappings (e.g., $y_{n_y+1} = x^{\BB}_1$) to absorb any direct $x^{\BB}$ dependence in $J$ or $g$, so the distinction between ``depends on $x^{\BB}$ through $y$'' and ``depends on $x^{\BB}$ directly'' is a modeling choice, not a structural limitation.

Despite this structure, problem~\eqref{eq:full_problem} resists standard solution methods for two reasons. First, the black-box nature of $f^{\BB}$ precludes gradient-based NLP solvers. Finite-difference gradients require $O(n_{\BB})$ evaluations of $f^{\BB}$ per iteration---prohibitive when each evaluation takes minutes to hours. Second, full-space surrogate-based optimization (EGO~\cite{jones1998efficient}, standard BO~\cite{frazier2018tutorial}) over $[x^{\WB}, x^{\BB}]$ builds a GP surrogate in $\mathbb{R}^{n_{\WB}+n_{\BB}}$, wasting samples on the known white-box response and suffering the curse of dimensionality as $n_{\WB}+n_{\BB}$ grows.
...
\For{$i = 1, \ldots, n_\text{init}$}
    ...
    \State Solve inner NLP: $x^{\WB\star}_i = \arg\min_{x^{\WB}} J(x^{\WB}, y_i, x^{\BB}_i)$ s.t.\ $f^{\WB}(x^{\WB}, y_i)=0$, $g(x^{\WB}, y_i, x^{\BB}_i) \leq 0$
...
\For{$t = n_\text{init}+1, \ldots, n_\text{init}+N$}
    ...
    \State Solve inner NLP $\to x^{\WB\star}_t$, record $J_t$
...
\begin{figure}[htbp]
    ...
    \end{tikzpicture}
    \caption{Flow of the multi-scale Bayesian optimization framework. The outer loop uses BO to select black-box variables $x^{\BB}$, which are evaluated through the black-box model to produce parameters $y$. The inner loop solves an NLP over white-box variables $x^{\WB}$ given $x^{\BB}$ and $y$, returning the optimal objective $J(x^{\WB\star}, y, x^{\BB})$ to update the BO surrogate.}
    \label{fig:multiscale_diagram}
\end{figure}

\subsection{Surrogate Model and Acquisition Function}\label{subsec:surrogate_and_acquisition}

The GP surrogate in Algorithm~\ref{alg:bilevel_bo} models the scalar mapping $x^{\BB} \mapsto J(x^{\WB\star}(x^{\BB}), f^{\BB}(x^{\BB}), x^{\BB})$---a function of $x^{\BB}$ alone. We use a Gaussian process with an automatic relevance determination (ARD) radial basis function (RBF) kernel, where a separate length scale per dimension allows the GP to adapt to anisotropic landscapes. The outer loop selects the next evaluation by maximizing Expected Improvement (EI) with exploration parameter $\xi \geq 0$. Kernel and acquisition function details are in Appendices~\ref{sec:gp_details} and~\ref{sec:acquisition_functions}; here we emphasize that both are standard---the gains reported in Section~\ref{sec:results} come from problem structure, not from novel surrogate or acquisition choices.
...
\noindent The proof is in Appendix~\ref{sec:proof_dimensionality}. The key consequence is threefold:

\emph{Exact constraint satisfaction.} Constraints $g(x^{\WB}, y, x^{\BB}) \leq 0$ are handled by the inner NLP solver, not by penalty functions or chance constraints. The GP never needs to learn the constraint boundary. When the inner NLP converges to a globally feasible point (Assumption~\ref{ass:separability}(ii)), feasibility is guaranteed; in practice, local solvers such as SLSQP with multi-start may find infeasible local optima on non-convex inner problems, as discussed in Section~\ref{sec:discussion}.
...
Table~\ref{tab:method_comparison} summarizes the distinctions. Both bilevel BO and COBALT achieve the same input dimensionality reduction to $n_{\BB}$. The distinction is in the surrogate output (scalar vs.\ multi-output GP) and constraint handling (exact NLP vs.\ moment approximation), not the input dimension. COBALT and BOCF surrogate intermediate black-box outputs $y$ and propagate uncertainty through the white-box equations, requiring multi-output GPs and moment approximations. Our method surrogates the scalar optimal objective $J(x^{\WB\star})$ directly, at the cost of an NLP solve per outer iteration. When the white-box model is moderately sized and the black-box evaluation dominates cost, this overhead is negligible. The approaches are complementary: COBALT handles $y$-dependent constraints natively and enables risk-averse decisions via uncertainty propagation; BOCF handles non-separable composite functions. Our method trades their generality for simplicity and exactness when Assumption~\ref{ass:separability} holds.
...
No existing benchmark suite targets the separable grey-box structure of Assumption~\ref{ass:separability}: an outer black-box mapping $y = f^{\BB}(x^{\BB})$ coupled with an inner white-box optimization over $x^{\WB}$, subject to inequality constraints. Existing suites for composite BO~\cite{astudillo2019bocf} do not separate variables; those used in grey-box BO~\cite{paulson2022cobalt} contain only a handful of problems; and standard global optimization testbeds (CEC, COCO/BBOB) lack bilevel structure entirely. We propose a suite of 13 problems spanning synthetic test functions and engineering domains, summarized in Table~\ref{tab:benchmark_summary}. Full mathematical formulations are in Appendix~\ref{sec:test_problems}.
...
Adapted from Gardner et al.~\cite{gardner2014} and Ariafar et al.~\cite{Ariafar2019}, these 2D problems minimize the constraints of these problems create small, disconnected feasible regions. The two-dimensional structure permits direct visualization of the search behavior (Section~\ref{subsec:sfr_results}).
...
The first solver, \textbf{black-box NLP}, treats $f^{\BB}$ as a numerically differentiable function and applies multi-start SLSQP over the full variable space $[x^{\WB}, x^{\BB}]$. We use 20 random restarts drawn from a Latin hypercube design. This solver provides an ``oracle'' baseline: it has gradient access (via finite differences) to both the white-box and black-box models. In practice, finite-differencing the black box requires $O(n_{\BB})$ evaluations per gradient step, so this approach is expensive when $f^{\BB}$ is costly.

The second solver, \textbf{black-box BO}, applies standard Bayesian optimization over the full $(n_{\WB}+n_{\BB})$-dimensional space. A single GP surrogate models the mapping $[x^{\WB}, x^{\BB}] \mapsto J$. Constraints are handled via a penalty function. This solver ignores the separable structure entirely and serves as the BO baseline.

The third solver, \textbf{bilevel BO}, is the proposed method. It places the GP surrogate over only the $n_{\BB}$-dimensional black-box space and solves the white-box subproblem exactly via NLP at each iteration, as described in Algorithm~\ref{alg:bilevel_bo}.
...
SFR-2 (Figure~\ref{fig:sfr2_search}) introduces a nonlinear black-box function, making the mapping from $x^{\BB}$ to the feasible objective landscape more complex. The same mechanism applies: the outer loop proposes $x^{\BB}$, the black box computes $y$, and the inner NLP finds the best feasible $x^{\WB}$. The GP surrogate now models a one-dimensional function $x^{\BB} \mapsto J(x^{\WB\star})$ rather than the two-dimensional objective-plus-constraint landscape that black-box BO must learn.

% SFR convergence curves
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figs/sfr_convergence.eps}
    \caption{Regret convergence for the two Small-Feasible-Region problems ($n_\text{init}=50$, best $\xi$, mean $\pm 1$ std over 10 repetitions). The dashed line marks the end of initialization. Bilevel BO (red) converges 1--2 orders of magnitude below black-box BO (blue) on both problems.}
...
\begin{figure}[htbp]
    ...
    \caption{Log$_{10}$ regret convergence curves for the 11 benchmark problems not shown in Figure~\ref{fig:sfr_convergence}. Bilevel BO (red) converges 1--4 orders of magnitude below black-box BO (blue) on every problem. Settings: $n_\text{init}=50$, best $\xi$ per problem, mean $\pm 1$ std over 10 repetitions.}
...
Bilevel BO achieves 14$\times$--277{,}000$\times$ lower regret than black-box BO across all 13 problems. The gap opens early and widens: bilevel BO separates from black-box BO within 20--50 iterations on most problems. The largest gains (Heat-Exchanger: 277{,}000$\times$, Distillation: 173{,}000$\times$) occur where tight constraints punish the penalty-based approach---the same mechanism illustrated by the SFR figures, but now in five dimensions where the advantage of exact constraint satisfaction compounds with dimensionality reduction. The smallest gains (Rosen-Suzuki: 14$\times$, Evaporator: 19$\times$) occur where the inner NLP has multiple local optima or the constraint set is loose.

The NLP baseline fails on two problems: Rastrigin, where multimodality traps gradient-based solvers, and Small-Feasible-Region-2, where the narrow feasible region defeats finite-difference gradients. Bilevel BO matches or approaches NLP quality on the remaining 11 problems using $\sim$250 black-box evaluations versus 900--3{,}000 for the 20-restart NLP. All 13 pairwise comparisons (bilevel vs.\ black-box BO) are statistically significant ($p < 0.01$, Wilcoxon signed-rank test, 10 repetitions).
...
\subsubsection{Dimensionality Scaling}\label{subsec:dimensionality_scaling}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{figs/dimensionality_scaling.pdf}
    \caption{Regret ratio (BB-BO / Bi-BO) vs.\ total problem dimension. The advantage of bilevel BO grows with dimension: 2D problems show 76--6{,}500$\times$ improvement; 5D problems show up to 277{,}000$\times$.}
    \label{fig:dimensionality_scaling}
\end{figure}

The upward trend in Figure~\ref{fig:dimensionality_scaling} is expected---bilevel BO searches $n_{\BB}=1$--2 dimensions regardless of total problem size---but the spread at 5D is instructive: constraint activity and landscape ruggedness cause the ratio to vary from 19$\times$ (Evaporator) to 277{,}000$\times$ (Heat-Exchanger) at the same total dimension. Figure~\ref{fig:scatter_bb_vs_bi} confirms that this dominance holds across all hyperparameter settings: every (problem, $n_\text{init}$, $\xi$) configuration lies below the diagonal.

% Scatter plot
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{figs/scatter_bb_vs_bi.pdf}
    \caption{Scatter plot of final regret: black-box BO vs.\ bilevel BO. Each point is one (problem, $n_\text{init}$, $\xi$) configuration. All points lie below the diagonal.}
    \label{fig:scatter_bb_vs_bi}
\end{figure}

\subsubsection{Computational Cost}\label{subsec:computational_cost}

Both BO methods require $\sim$20--30s wall time for 200 iterations on 12 of 13 problems, as the lower-dimensional GP in bilevel BO roughly offsets the inner NLP cost. The exception is Williams-Otto (111s vs.\ 26s), where the inner NLP solves a 6-species steady-state mass balance at each iteration. Both BO methods use $\sim$205 black-box evaluations---an order of magnitude fewer than the 900--3{,}000 required by the multi-start NLP baseline.

\subsubsection{Hyperparameter Robustness}\label{subsec:robustness}
...
Figure~\ref{fig:ninit_robustness} shows that bilevel BO performance is nearly flat across $n_\text{init} \in \{1, 5, 20, 50\}$ (numerical values in Appendix Table~\ref{tab:ninit_sensitivity}). Even $n_\text{init}=1$---a single initial sample---performs comparably to $n_\text{init}=50$, so fewer expensive initial evaluations suffice when $f^{\BB}$ is costly.

Bilevel BO also dominates black-box BO across all values of the EI exploration parameter $\xi$ tested. No single $\xi$ is universally optimal---the best setting ranges from $\xi=0.001$ (CSTR, Distillation, Williams-Otto) to $\xi=0.5$ (Small-Feasible-Region-1, Membrane)---but adaptive $\xi$ strategies are not critical for the bilevel approach (Appendix Figure~\ref{fig:xi_heatmap}).
...
Bilevel BO achieves 14$\times$--277{,}000$\times$ lower regret than black-box BO on every problem tested, with comparable computational cost. The source of this advantage is structural, not algorithmic: we use the same GP kernel, the same acquisition function, and the same optimizer as the black-box baseline. The gains come entirely from exploiting the separable problem structure---reducing the surrogate dimension and satisfying constraints exactly. Three additional analyses---feasibility rates, a two-factor decomposition of the advantage, and sample efficiency crossover---provide mechanistic insight into when and why the bilevel decomposition helps.

\noindent\textbf{Constraint satisfaction and wasted evaluations.}
A key mechanism underlying the regret gap is that black-box BO wastes evaluations on infeasible queries. Figure~\ref{fig:feasibility_rates} reports the fraction of all 250 evaluations that land in feasible regions, reconstructed from stored search histories. Black-box BO wastes 6--72\% of its budget on infeasible points depending on the problem, with Small-Feasible-Region (72\%), Batch-Reactor (71\%), and Evaporator (58\%) being the worst cases. Bilevel BO achieves 100\% feasibility on 8 of 12 constrained problems because the inner NLP handles constraint satisfaction exactly. On the remaining four problems (Batch-Reactor, Evaporator, Small-Feasible-Region, PSA), bilevel BO feasibility ranges from 59--91\%, reflecting cases where the inner NLP converges to boundary solutions with slight numerical infeasibility or where some $y = f^{\BB}(x^{\BB})$ values render the inner problem infeasible entirely. This is not a limitation of the bilevel architecture itself but of the local NLP solver; deterministic global inner solvers would restore the 100\% guarantee.
...
\end{equation}
where $n$ is the current iteration and $N$ is the total budget. The dynamic scaling factor $s_n$ starts large (emphasizing EI-driven exploration) and decreases toward zero (shifting weight to the GP mean for exploitation). For maximization, the sign of $\mu(x)$ is flipped.
...
%% ============================================================================
%% APPENDIX D: NLP BASELINE DETAILS
%% ============================================================================
\section{NLP Baseline Details}\label{sec:nlp_details}

The black-box NLP baseline solves the full problem~\eqref{eq:full_problem} directly using \texttt{scipy.optimize.minimize} with the SLSQP method. Because SLSQP is a local solver and the problems are generally non-convex, we use a multi-start strategy with 20 random restarts drawn from a Latin hypercube over the full variable bounds $\mathcal{X}^{\WB} \times \mathcal{X}^{\BB}$.

\subsection{Gradient Computation}\label{subsec:nlp_gradients}

SLSQP requires gradient information. For the white-box variables $x^{\WB}$, the objective $J$ and constraints $g$ are differentiable and \texttt{scipy} computes gradients via finite differences. For the black-box variables $x^{\BB}$, the gradients of the composite function $J(x^{\WB}, f^{\BB}(x^{\BB}))$ with respect to $x^{\BB}$ also require finite differences through $f^{\BB}$. Each gradient evaluation thus requires $O(n_{\BB})$ additional calls to $f^{\BB}$. Over 20 multi-start runs, the total number of $f^{\BB}$ evaluations is typically 900--3{,}000, depending on the problem and the number of iterations each SLSQP run takes to converge.

\subsection{Constraint Handling}\label{subsec:nlp_constraints}

SLSQP handles inequality constraints natively through its sequential quadratic programming formulation. However, convergence to feasible points is not guaranteed for non-convex constraint sets. We observed that on several constrained problems, some multi-start runs converge to constraint-violating local optima. The NLP baseline reports the best objective across all 20 starts without checking feasibility, which means its reported regret may be optimistic when the best NLP solution is slightly infeasible.

In contrast, both BO methods track feasibility explicitly. For black-box BO, constraints are handled via a penalty of $10^6$ added to the objective when any $g_i(x) > 0$. For bilevel BO, the inner NLP solver checks constraint satisfaction and returns a penalty value when the inner problem is infeasible for a given $y$.

\subsection{Sample Efficiency Comparison}\label{subsec:nlp_sample_efficiency}

Table~\ref{tab:sample_efficiency} compares the number of black-box evaluations across solvers. The NLP baseline uses an order of magnitude more $f^{\BB}$ evaluations than either BO method, making it impractical when each evaluation is truly expensive (e.g., DFT calculations taking hours).
...
\midrule
Black-box NLP & 900--3{,}000 & 20 restarts $\times$ finite-diff.\ gradients \\
...
Both BO methods call $f^{\BB}$ exactly once per iteration, so the total sample budget is $n_\text{init} + N$ where $N=200$ is the number of BO iterations. With $n_\text{init} = 5$ (the default), this gives 205 evaluations. Bilevel BO additionally solves an inner NLP at each iteration, but this uses only the white-box model $f^{\WB}$ (which is cheap) and does not require additional calls to $f^{\BB}$.

\subsection{Failure Modes}\label{subsec:nlp_failures}

The NLP baseline fails in two distinct ways. First, on multimodal problems (Rastrigin), gradient-based optimization converges to local minima. The 20 multi-start restarts mitigate this but do not eliminate it: the NLP achieves a final regret of 3.38 on Rastrigin, worse than both BO methods. Second, on problems with narrow feasible regions or nonlinear black-box coupling (Small-Feasible-Region-2), finite-difference gradients through $f^{\BB}$ are inaccurate near constraint boundaries. The NLP reports a regret of 0.055 on this problem, while bilevel BO achieves effectively zero regret.
...
We used three complementary approaches. First, we ran differential evolution (DE) over the full variable space $[x^{\WB}, x^{\BB}]$ with a population of 100 and 1{,}000 generations. Second, we ran bilevel DE---differential evolution over $x^{\BB}$ with the inner NLP solved exactly for each candidate---to exploit the separable structure during verification. Third, we ran multi-start NLP with 1{,}000 random restarts.

For each problem, we compared the best solutions from all three approaches. When all three methods agreed to within $10^{-4}$, we accepted the optimum. When they disagreed, we investigated manually by examining the objective landscape and constraint activity at the candidate optima.

\subsection{Feasibility Checks}\label{subsec:feasibility_checks}

For constrained problems, we verified that the reported optimal solution satisfies all inequality constraints $g_i(x^{\WB\star}, y^\star) \leq 0$ to within a tolerance of $10^{-6}$. We also checked that no nearby feasible point achieves a better objective, by perturbing the optimal solution along the active constraint boundaries and confirming that the objective worsens.
...
\begin{table}[htbp]
\centering
\caption{Verified global optima for all 13 benchmark problems.}
\label{tab:verified_optima}
\begin{tabular}{@{}lrll@{}}
\toprule
\textbf{Problem} & $J^\star$ & $x^{\WB\star}$ & $x^{\BB\star}$ \\
\midrule
Small-Feasible-Region 1 & 0.2532 & $[4.712]$ & $[0.253]$ \\
...
\midrule
CSTR & 92.32\% & $[647.2, 20.0, 5.0]$ & $[-1.047, 0.200]$ \\
Heat-Exchanger & 1000.0 & $[1.0, 0.1, 39.6]$ & $[50.0, 0.0]$ \\
PSA & $-0.6433$ & $[3.97, 0.1, 10.0]$ & $[6.04, 19.6]$ \\
Batch-Reactor & $-1.749$ & $[500, 4.39, 5.0]$ & $[-1.0, 2.0]$ \\
Distillation & 11{,}758 & $[1.0, 11.0, 0.5]$ & $[20.0, 10.0]$ \\
Evaporator & $-1.959$ & $[4.09, 19.1, 5.0]$ & $[120.5, 2.0]$ \\
Membrane & 11.00 & $[10.0, 0.1, 1.0]$ & $[0.30, 5.13]$ \\
Williams-Otto & 5.460 & $[581.6, 50.0, 50.0]$ & $[12.6, 6.11]$ \\
\bottomrule
\end{tabular}
\end{table}
...
If the inner NLP is infeasible for some $\tilde{y} \in \text{range}(f^{\BB})$---i.e., no $x^{\WB} \in \mathcal{X}^{\WB}$ satisfies $f^{\WB}(x^{\WB}, \tilde{y}) = 0$ and $g(x^{\WB}, \tilde{y}, \tilde{x}^{\BB}) \leq 0$---then no feasible point of~\eqref{eq:full_problem} exists at this $\tilde{x}^{\BB}$ either, so excluding it from the outer search does not lose optimality.

From (i) and (ii), the feasible sets project onto each other with preserved objectives, so $\inf(\text{bilevel}) = \inf(\text{original})$. Assumption~\ref{ass:separability}(ii) is used in step (i) to ensure the inner global optimum is attained, and in step (ii) to ensure the inner solution is globally optimal (not merely a local minimum). The surrogate in Algorithm~\ref{alg:bilevel_bo} models $x^{\BB} \mapsto J(x^{\WB\star}(x^{\BB}), f^{\BB}(x^{\BB}), x^{\BB})$, which is a function of $n_{\BB}$ variables.
\end{proof}
...
\emph{Infeasibility handling.} If the inner NLP is infeasible for some $y$---because the black-box outputs lead to a white-box problem with no feasible solution---we return a large penalty value. The acquisition function naturally steers away from these regions without requiring explicit constraint modeling at the outer level.

\emph{Modularity.} The inner NLP solver can be swapped (SLSQP, IPOPT, BARON) without changing the outer BO framework. Similarly, any acquisition function (EI, PI, LCB) can be used in the outer loop.
